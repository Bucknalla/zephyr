...
static int pmsa003i_read_with_timeout(const struct pmsa003i_config *cfg, uint8_t *buf, size_t len)
{
	...
	while (k_uptime_get() - start_time < timeout_ms) {
		...
		if (buf[0] == PMSA003I_REG_START_BYTE_1 && buf[1] == PMSA003I_REG_START_BYTE_2) {
			return 0;
		}
...
static int pmsa003i_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	if (checksum != read_buf[PMSA003I_CHECKSUM_LEN - 1]) {
		LOG_WRN("checksum mismatch (calc: 0x%02X, recv: 0x%02X)", checksum,
			read_buf[PMSA003I_CHECKSUM_LEN - 1]);
		return -EIO;
	}

	data->pm_1_0 = sys_get_be16(&read_buf[PMSA003I_REG_PM_1_0]);
	data->pm_2_5 = sys_get_be16(&read_buf[PMSA003I_REG_PM_2_5]);
	data->pm_10 = sys_get_be16(&read_buf[PMSA003I_REG_PM_10]);
